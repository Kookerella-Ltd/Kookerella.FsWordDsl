...
| `Xml.fs`/`Json.fs`'s `toDocument` output stays order-independent for `Styles`/`Numbering`/`TableStyles` (each sorted by `Id`) - a new field added to `Document` that's an ID-referenced catalog (not real document-order content) needs the same sort, or two semantically-identical documents can produce a spuriously different diff | Two `Document`s with the same content but reversed catalog list order must render byte-identical XML/JSON | `` Xml.toDocument produces deterministic, input-order-independent output for Styles, Numbering, and TableStyles `` (`XmlTests.fs`) and its JSON equivalent (`JsonTests.fs`) - mirrors the Excel sibling's own `` Xml.ofWorkbook produces deterministic, input-order-independent output `` test for `DefinedNames` |
